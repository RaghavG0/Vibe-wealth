      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            {navItems.map((item) => (
              <a
                key={item.name}
                href={item.href}
                className="text-vibe-gray-600 hover:text-vibe-purple-600 transition-colors duration-200 font-medium"
              >
                {item.name}
              </a>
            ))}
          ...
          <button
            ...
            className="md:hidden p-2 rounded-lg hover:bg-vibe-gray-100 transition-colors"
          >
            {isOpen ? (
            ...
            <div className="px-2 pt-2 pb-3 space-y-1">
              {navItems.map((item) => (
                <a
                  key={item.name}
                  href={item.href}
                  className="block px-3 py-2 text-vibe-gray-600 hover:text-vibe-purple-600 hover:bg-vibe-purple-50 rounded-lg transition-colors"
